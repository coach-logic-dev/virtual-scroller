...
export default class VirtualScroller {
   ...
   */
  constructor(getContainerElement, items, options = {}) {
    const {
      getState,
      setState,
      onStateChange,
    ...
    this.measureItemsBatchSize =
      measureItemsBatchSize === undefined ? 50 : measureItemsBatchSize;
    ...
    // `onItemFirstRender(i)` is deprecated, use `onItemInitialRender(item)` instead.
      ...
      this.onItemFirstRender = (item) => {
        console.warn(
          "[virtual-scroller] `onItemFirstRender(i)` is deprecated, use `onItemInitialRender(item)` instead."
        );
    ...
    if (this.scrollableContainer) {
      if (preserveScrollPositionAtBottomOnMount) {
        console.warn(
          "[virtual-scroller] `preserveScrollPositionAtBottomOnMount` option/property has been renamed to `preserveScrollPositionOfTheBottomOfTheListOnMount`"
        );
  ...
  getEstimatedItemHeight() {
    return (
      (this.itemHeights && this.itemHeights.getAverage()) ||
  ...
  getEstimatedItemsCount(height) {
    if (this.getEstimatedItemHeight()) {
      return Math.ceil(
        (height + this.getItemSpacing()) /
          (this.getEstimatedItemHeight() + this.getItemSpacing())
      );
    } else {
      return 1;
    }
  ...
  getEstimatedItemsCountOnScreen() {
    if (this.scrollableContainer) {
      return this.getEstimatedItemsCount(
        this.getMargin() * 2 + this.scrollableContainer.getHeight()
      );
    } else {
      return 1;
    }
  ...
  onMount() {
    console.warn(
      "[virtual-scroller] `.onMount()` instance method name is deprecated, use `.listen()` instance method name instead."
    );
    this.listen();
  }
  ...
  render() {
    console.warn(
      "[virtual-scroller] `.render()` instance method name is deprecated, use `.listen()` instance method name instead."
    );
    this.listen();
  }
  ...
  listen() {
    if (this.isRendered === false) {
      throw new Error(
        "[virtual-scroller] Can't restart a `VirtualScroller` after it has been stopped"
      );
    }
    ...
    this.removeScrollPositionListener = this.scrollableContainer.addScrollListener(
      this.updateScrollPosition
    );
    if (!this.bypass) {
      this.removeScrollListener = this.scrollableContainer.addScrollListener(
        this.onScroll
      );
      this.scrollableContainerUnlistenResize = this.scrollableContainer.onResize(
        this.onResize
      );
    }
    // Work around `<tbody/>` not being able to have `padding`.
    ...
    if (this.preserveScrollPositionOfTheBottomOfTheListOnMount) {
      ...
      this.scrollTo(
        0,
        this.getScrollY() +
          (this.scrollableContainer.getHeight() -
            this.preserveScrollPositionOfTheBottomOfTheListOnMount
  ...
  shouldUpdateLayoutOnScrollableContainerResize(event) {
    if (event && event.target === window) {
      ...
      if (
        ...
        this.getContainerElement().contains(document.fullscreenElement)
      ) {
        return false;
      }
  ...
  onUnmount() {
    console.warn(
      "[virtual-scroller] `.onUnmount()` instance method name is deprecated, use `.stop()` instance method name instead."
    );
    this.stop();
  }
  ...
  destroy() {
    console.warn(
      "[virtual-scroller] `.destroy()` instance method name is deprecated, use `.stop()` instance method name instead."
    );
    this.stop();
  }
  ...
  getVisibleItemIndexes(visibleAreaTop, visibleAreaBottom, listTopOffset) {
    ...
    while (i < this.getItemsCount()) {
      ...
      if (height === undefined) {
        log(
          `Item index ${i} lies within the visible area or its "margins", but its height hasn't been measured yet. Mark the item as "shown", render the list, measure the item's height and redo the layout.`
        );
    ...
    if (redoLayoutAfterRender && this.measureItemsBatchSize) {
      lastShownItemIndex = Math.min(
        lastShownItemIndex,
        firstNonMeasuredItemIndex + this.measureItemsBatchSize - 1
      );
    }
    return {
      firstShownItemIndex,
  ...
  getItemIndexes(visibleAreaTop, visibleAreaBottom, listTopOffset, listHeight) {
    const isVisible =
      listTopOffset + listHeight > visibleAreaTop &&
      listTopOffset < visibleAreaBottom;
    if (!isVisible) {
      log("Off-screen");
      return;
    }
  ...
  getBeforeItemsHeight(firstShownItemIndex, lastShownItemIndex) {
    ...
    while (i < firstShownItemIndex) {
      beforeItemsHeight +=
        this.itemHeights.get(i) || this.itemHeights.getAverage();
   ...
   */
  updateWillBeHiddenItemHeightsAndState(
    firstShownItemIndex,
    lastShownItemIndex
  ) {
  ...
  watchContainerElementCoordinates() {
    ...
    const check = () => {
      ...
      if (
        Date.now() - startedAt <
        WATCH_CONTAINER_ELEMENT_TOP_COORDINATE_MAX_DURATION
      ) {
        this.watchContainerElementCoordinatesTimer = setTimeout(
          check,
          WATCH_CONTAINER_ELEMENT_TOP_COORDINATE_INTERVAL
        );
      }
    };
    // Run the cycle.
  ...
  getShownItemIndexes() {
    ...
    this.latestLayoutVisibleAreaBottomAfterIncludingMargin =
  ...
  updateShownItemIndexes = () => {
    ...
    // in the list item height changing and `this.itemHeights[i]`
    // being stale, so it's updated here when hiding the item.
    this.updateWillBeHiddenItemHeightsAndState(
      firstShownItemIndex,
      lastShownItemIndex
    );
    // Debugging.
    ...
    log(
      "Average item height (calculated on previous render)",
    ...
    if (this.firstSeenItemIndex !== undefined) {
      if (
        firstShownItemIndex > this.lastSeenItemIndex + 1 ||
  ...
  restoreScroll = () => {
    ...
    const newVisibleAreaTop = this.getItemElement(index).getBoundingClientRect()
      .top;
    ...
    if (reason === "scroll") {
      ...
      const forceRender =
        ...
        (this.latestLayoutVisibleAreaBottomAfterIncludingMargin !== undefined &&
          ...
          this.getScrollY() + this.scrollableContainer.getHeight() >
            this.latestLayoutVisibleAreaBottomAfterIncludingMargin &&
      ...
      if (!forceRender) {
        return (this.onUserStopsScrollingTimeout = setTimeout(
  ...
  setItems(newItems, options = {}) {
    ...
    const { items: previousItems } = this.getState();
    let {
      firstShownItemIndex,
      lastShownItemIndex,
      beforeItemsHeight,
    ...
    if (isIncrementalUpdate) {
      ...
      log(
        "Items have changed, and it's not a simple append and/or prepend: rerender the entire list from scratch."
      );
    ...
    this.preserveScrollPositionOnPrependItems =
      options.preserveScrollPositionOnPrependItems ||
  ...
  if (Array.isArray(arr1) && Array.isArray(arr2) && arr1.length === arr2.length) {
    return (
      arr1
        ...
        .concat(arr2.filter((x) => !arr1.includes(x))).length === 0
    );
  }
  return false;
};
...
export function getItemsDiff(previousItems, newItems) {
  ...
  if (previousItems.length > 0) {
    firstPreviousItemIndex = newItems.findIndex(
      (item) =>
        (item.id && item.id === previousItems[0].id) ||
        isArrayEqual(item, previousItems[0])
    );
    if (firstPreviousItemIndex >= 0) {
      if (
        arePreviousItemsPreserved(
          previousItems,
          newItems,
          firstPreviousItemIndex
        )
      ) {
        lastPreviousItemIndex =
          firstPreviousItemIndex + previousItems.length - 1;
      }
    }
  }
  const isIncrementalUpdate =
    firstPreviousItemIndex >= 0 || lastPreviousItemIndex >= 0;
  if (isIncrementalUpdate) {
    return {
